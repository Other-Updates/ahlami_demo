...
<div class="panel panel-flat">
    <div class="panel-heading">
        <h5 class="panel-title">Manage Events
            <?php if ($this->user_auth->is_action_allowed('events', 'events', 'add')): ?>
                <a href="<?php echo base_url(); ?>events/add"><span class="label bg-success">Add New</span></a>
            <?php endif; ?>
        </h5>
    ...
    <table id="eventTable" class="table table-bordered table-striped table-hover responsive dataTable no-footer dtr-inline status-cntrb acccent invcent cente cente1 cente2" width="100%">
        <thead>
            <tr>
                <th >S.No</th>
                <th >Event&nbsp;Type</th>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        var table = $('#eventTable').DataTable({
            ...
            'ajax': {
                'url': '<?php echo base_url() . 'events/events_ajaxList'; ?>',
                'type': 'POST',
            },
            'columnDefs': [{
                    'targets': 0,
    ...
    $('body').on('click', '.delete_event', function () {
        ...
        $.confirm({
            ...
            confirm: function (button) {
                $.ajax({
                    url: '<?php echo base_url(); ?>events/events/delete',
                    method: 'POST',
                    data: {id: id},
