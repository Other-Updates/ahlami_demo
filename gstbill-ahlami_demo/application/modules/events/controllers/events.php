...
class Events extends MX_Controller {
    ...
    function add() {
        ...
        $this->template->write_view('content', 'events/add_event', $data);
        $this->template->render();
    }
    ...
    function edit($id) {
        ...
        if ($this->input->post()) {
            ...
            $invited_mem = $this->event_model->get_event_invited_members_by_event_id($id);
            $already_invited_list = $invited_mem[0]['invited_members'];
            $already_invited = str_replace("[", "", $already_invited_list);
            $already_invited2 = str_replace("[", "", $already_invited);
            $already_invited_mem = str_replace("]", "", $already_invited2);
            $invited_members = array ();
            if (!empty($invited_members_arr)) {
                $members_list = $this->members_model->get_members_by_list($invited_members_arr, $already_invited_mem);
                if (!empty($members_list)) {
                    foreach ($members_list as $list) {
                        $invited_members[$list['id']] = $list['email_address'];
                    }
        ...
        $this->template->write_view('content', 'events/edit_event', $data);
        $this->template->render();
    }
    ...
    function view_bk($id) {
        ...
        $data['event_dynamic_fields'] = $event_dynamic_fields;
        $this->template->write_view('content', 'events/view_event', $data);
        $this->template->render();
    }
    ...
    function view($id) {
        ...
        $this->template->write_view('content', 'events/view_event_new', $data);
        $this->template->render();
    }
    ...
    function delete() {
        ...
        if ($delete) {
            $this->session->set_flashdata('flashSuccess', 'Events successfully deleted!');
            echo '1';
        } else {
    ...
    function event_invitation_mail($id, $invited_members = array ()) {
        ...
        $htmlContent .= $this->load->view('events/event_invitation_mail.php', $data, TRUE);
        $invited_members_list = array_values($invited_members);
        $invited_members_lists = implode(',', $invited_members_list);
        $invite_members_details = array ($invited_members_lists);
        $this->email->from('ftwoftesting@gmail.com', 'Events');
        foreach ($invite_members_details as $to) {
...
//                                $option_text .= '<div class="radio"><label><input type="radio" name="dynamic[' . $fieldID . ']" value="' . $option['value'] . '" ' . $is_checked . '>' . $option['label'] . '</label></div>';
    ...
    function invited_members($id) {
        ...
        $this->template->write_view('content', 'events/invited_members', $data);
        $this->template->render();
    }
    ...
    function accept_members($id) {
        ...
        $this->template->write_view('content', 'events/accept_members', $data);
        $this->template->render();
    }
    ...
    function reject_members($id) {
        ...
        $this->template->write_view('content', 'events/reject_members', $data);
        $this->template->render();
    }
    ...
    function events_ajaxList() {
        ...
        foreach ($list as $ass) {
            ...
            $row[] = '<a class="btn btn-primary btn-xs" href="' . $this->config->item('base_url') . 'events/events/invited_members/' . $ass['id'] . '">' . $invited_count . '</a>';
            $row[] = '<a class="btn btn-success btn-xs" href="' . $this->config->item('base_url') . 'events/events/accept_members/' . $ass['id'] . '">' . $accept_count . '</a>';
            $row[] = '<a class="btn btn-danger btn-xs" href="' . $this->config->item('base_url') . 'events/events/reject_members/' . $ass['id'] . '">' . $reject_count . '</a>';
            ...
            $row[] = $status;
            $action_str = '';
            if ($this->user_auth->is_action_allowed('events', 'events', 'view')) {
                $action_str .= '<a href="' . $this->config->item('base_url') . 'events/view/' . $ass['id'] . '" class="btn btn-success btn-xs" title="View"><i class="glyphicon glyphicon-eye-open"></i></a>&nbsp;';
            }
            if ($this->user_auth->is_action_allowed('events', 'events', 'edit')) {
                $action_str .= '<a href="' . $this->config->item('base_url') . 'events/edit/' . $ass['id'] . '" class="btn btn-info btn-xs" title="Edit"><i class="glyphicon glyphicon-edit"></i></a>&nbsp;';
            }
            if ($this->user_auth->is_action_allowed('events', 'events', 'delete')) {
                $action_str .= '<a href="javascript:void(0);" event_id="' . $ass['id'] . '" event_id="' . $ass['id'] . '" class="btn btn-danger btn-xs delete_event" title="Delete"><i class="glyphicon glyphicon-trash"></i></a>';
            }
            if (!empty($action_str)) {
                $row[] = $action_str;
            }
