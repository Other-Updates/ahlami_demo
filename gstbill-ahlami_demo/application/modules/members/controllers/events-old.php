...
>>>>>>> .r5216
        ...
        $data['event_dynamic_fields'] = $event_dynamic_fields;
        $this->template->write_view('content', 'members/view_event', $data);
        $this->template->render();
    }
    ...
    function event_invitation_mail($id, $invited_members = array ()) {
        ...
        $data['event'] = $this->event_model->get_event_details_by_id($id);
        $data['event_invitations'] = $this->event_model->get_event_invitations_by_event_id($id);
        $attach_img = $data['event_invitations'];
        $attach = $attach_img[0]['img_path'];
        ...
        $htmlContent .= $this->load->view('events/event_invitation_mail', $data, TRUE);

        $invited_members_list = array_values($invited_members);
        $invited_members_list = implode(',', $invited_members_list);
        $this->email->from('ftwoftesting@gmail.com', 'Events');
        //$this->email->to($invited_members_list);
...
//            $row[] = ucfirst($ass['street_name']);
            ...
            if ($ass['approved_status'] == '1') {
                ...
                $approved_status = '<span class="label label-danger">Disapproved</span>';
            }
            $row[] = $approved_status;
            if ($ass['status'] == '1') {
            ...
            $row[] = $status;
            if ($this->user_auth->is_action_allowed('members', 'members', 'view', 'edit', 'delete')) {
                $row[] = '<a href="' . $this->config->item('base_url') . 'events/view/' . $ass['id'] . '" class="btn btn-success btn-xs" title="View"><i class="glyphicon glyphicon-eye-open"></i></a>&nbsp;<a href="' . $this->config->item('base_url') . 'members/events/edit/' . $ass['id'] . '" class="btn btn-info btn-xs" title="Edit"><i class="glyphicon glyphicon-edit"></i></a>&nbsp;<a href="javascript:void(0);" event_id="' . $ass['id'] . '" event_id="' . $ass['id'] . '" class="btn btn-danger btn-xs delete_event" title="Delete"><i class="glyphicon glyphicon-trash"></i></a>';
            }
            $data[] = $row;
        }
