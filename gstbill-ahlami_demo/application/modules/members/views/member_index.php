<?php $theme_path = $this->config->item('theme_locations') . 'event'; ?>
<div class="panel panel-flat">
    <div class="panel-heading">
        <h5 class="panel-title">Manage Members
            <?php if ($this->user_auth->is_action_allowed('members', 'members', 'add')): ?>
                <a href="<?php echo base_url(); ?>members/user/add" title="Add New"><span class="label bg-success">Add New</span></a>
            <?php endif; ?>
        </h5>
        <div class="heading-elements">
            <ul class="icons-list">
                <li><a data-action="collapse"></a></li>
            </ul>
        </div>
    </div>
    <table class="table datatable-basic table-bordered table-striped table-hover">
        <thead>
            <tr>
                <th>S.No</th>
                <th>Name</th>
                <th>Username</th>
                <th>Email</th>
                <th>Mobile</th>
                <th>Family Name</th>
                <th>Relation</th>
                <th>Street</th>
                <th>City</th>
                <th>Approved Status</th>
                <th>Status</th>
                <?php if ($this->user_auth->is_action_allowed('members', 'members', 'edit') || $this->user_auth->is_action_allowed('members', 'members', 'delete')): ?>
                    <th class="text-center">Actions</th>
                <?php endif; ?>
            </tr>
        </thead>
        <tbody>
            <?php
            if (!empty($members)) {
                $s = 1;
                foreach ($members as $list) {
                    ?>
                    <tr class="mem_del<?php echo $list['id']; ?>">
                        <td><?php echo $s; ?></td>
                        <td><?php echo ucfirst($list['firstname']) . ' ' . ucfirst($list['lastname']); ?></td>
                        <td><?php echo ucfirst($list['username']); ?></td>
                        <td><?php echo $list['email_address']; ?></td>
                        <td><?php echo $list['mobile_number']; ?></td>
                        <td><?php echo ucfirst($list['group_name']); ?></td>
                        <td><?php echo ucfirst($list['relation']); ?></td>
                        <td><?php echo ucfirst($list['street_name']); ?></td>
                        <td><?php echo ($list['city'] == 1) ? 'Kayalpattinam' : 'Others' ?></td>
                        <?php if ($list['approved_status'] == 1) { ?>
                            <td><span class="label label-success"><?php echo 'Approved' ?></span></td>

                        <?php } elseif ($list['approved_status'] == 2) { ?>
                            <td><span class="label label-default"><?php echo 'Hold' ?></span></td>
                        <?php } elseif ($list['approved_status'] == 0) { ?>
                            <td><span class="label label-danger"><?php echo 'Rejected' ?></span></td>
                        <?php } ?>

                        <td><span class="label label-<?php echo ($list['status'] == 1) ? 'success' : 'default'; ?>"><?php echo ($list['status'] == 1) ? 'Active' : 'Inactive'; ?></span></td>
                        <?php if ($this->user_auth->is_action_allowed('members', 'members', 'edit') || $this->user_auth->is_action_allowed('members', 'members', 'delete')): ?>
                            <td class="text-center">
                                <?php if ($this->user_auth->is_action_allowed('members', 'members', 'edit')): ?>
                                    <a href="<?php echo base_url(); ?>members/user/edit/<?php echo $list['id']; ?>" class="btn btn-info btn-xs" title="Edit"><i class="glyphicon glyphicon-edit"></i></a>
                                <?php endif; ?>
                                <?php if ($this->user_auth->is_action_allowed('members', 'members', 'delete')): ?>
                                    <a href="javascript:void(0);" class="btn btn-danger btn-xs delete_member" onclick="delete_mem(<?php echo $list['id']; ?>)" title="Delete"><i class="glyphicon glyphicon-trash"></i></a>
                                    <?php endif; ?>
                            </td>
                        <?php endif; ?>
                    </tr>
                    <?php
                    $s++;
                }
            }
            ?>
        </tbody>
    </table>
</div>

<script type="text/javascript">
    $(document).ready(function () {
        $('th.sorting_disabled').css('width', '135px');

    });


    function delete_mem(id) {
        $.confirm({
            title: 'Delete',
            text: 'Are you sure to Delete?',
            confirm: function (button) {
                $.ajax({
                    type: 'POST',
                    url: '<?php echo base_url(); ?>members/user/delete/' + id,
                    success: function (data) {
                        if (data == 1) {
                            session_message();
                            $('.mem_del' + id + '').remove();
                        }
                    }
                });
            },
            cancel: function (button) {
            },
            confirmButton: 'Yes',
            cancelButton: 'No'
        });
    }
</script>