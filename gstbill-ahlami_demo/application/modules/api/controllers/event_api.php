...
Class Event_api extends MX_Controller {
    ...
    function api_add_contacts() {
        $contact_info = $this->input->post('contact_info');
        if (!empty($contact_info)) {
            $contact_info['created_date'] = date('Y-m-d H:i:s');
            $insert_id = $this->event_api_model->insert_contact_details($contact_info);
            if (!empty($insert_id)) {
                ...
                $email_id = $this->event_api_model->get_contact_receive_mail_id();


                $htmlContent .= $this->load->view('site_cms/contact_email', $data, TRUE);
                $this->email->from('ftwoftesting@gmail.com', 'Events');
//                $this->email->to('kavithabui2k18@gmail.com');
                $this->email->to($email_id);
                $this->email->subject('New Contact received from Events Website');
                $this->email->message($htmlContent);
                $this->email->send();
                $output = array('status' => 'success', 'message' => 'Contact details successfully added!');
                echo json_encode($output);
            } else {
                $output = array('status' => 'error', 'message' => 'Contact details not added!');
                echo json_encode($output);
            }
        } else {
            $output = array('status' => 'error', 'message' => 'Invalid data!');
    ...
    function api_add_member_image() {
        ...
        if (!empty($_FILES['member_image']['name'])) {
            $_FILES['member_image'] = array(
                'name' => $_FILES['member_image']['name'],
                'type' => $_FILES['member_image']['type'],
                'tmp_name' => $_FILES['member_image']['tmp_name'],
                'error' => $_FILES['member_image']['error'],
        ...
        echo json_encode(array('member_image' => $profile_image));
    }

    function api_add_member() {
        $member_info = $this->input->post('member_info');
        //$member_info = $this->input->post();
        //echo '<pre>';
        //print_r($member_info);
        //exit;
        if (!empty($member_info)) {
            $password_nor = $member_info['password'];
            $member_info['password'] = md5($member_info['password']);
            $member_info['member_id'] = $this->increment_model->get_increment_code('member_code');
            $dob = explode(' ', $member_info['dob']);
            $dob_date = explode('/', $dob[0]);
            $dob_formatted = $dob_date[2] . '-' . $dob_date[1] . '-' . $dob_date[0];
            $member_info['dob'] = $dob_formatted;
            if ($member_info['on_kayalpattinam'] != 'temporary') {
                $member_info['duration_from'] = NULL;
                $member_info['duration_to'] = NULL;
            } else {
                $dur_from = explode(' ', $member_info['duration_from']);
                $dur_from_date = explode('/', $dur_from[0]);
                $dur_from_formatted = $dur_from_date[2] . '-' . $dur_from_date[1] . '-' . $dur_from_date[0];

                $dur_to = explode(' ', $member_info['duration_to']);
                $dur_to_date = explode('/', $dur_to[0]);
                $dur_to_formatted = $dur_to_date[2] . '-' . $dur_to_date[1] . '-' . $dur_to_date[0];

                $member_info['duration_from'] = $dur_from_formatted;
                $member_info['duration_to'] = $dur_to_formatted;
            }
            $member_info['created_date'] = date('Y-m-d H:i:s');

            $member_info['user_type_id'] = 2;
            $member_info['approved_status'] = 2;

            $insert_id = $this->event_api_model->insert_member($member_info);
            ...
            if (!empty($insert_id)) {
                ...
                $email_id = $member_info['email_address'];
                $data['password'] = $password_nor;

                // TRIGGER CONTACT EMAIL
                $htmlContent .= $this->load->view('site_cms/member_credentials_email.php', $data, TRUE);
                $this->email->from('ftwoftesting@gmail.com', 'Events');
                $this->email->to($email_id);
                $this->email->subject('Your Member Registration credentials from Events Website');
                $this->email->message($htmlContent);
                $this->email->send();
                $output = array('status' => 'success', 'message' => 'New Memeber successfully registered!');
                echo json_encode($output);
