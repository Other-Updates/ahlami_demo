...
Class Event_api extends MX_Controller {
    ...
    function api_add_event() {
        $json_input = file_get_contents('php://input'); // JSON Input
        //$json_input = '{"event_info":{"event_type_id":"1","event_name":"Yuvi Marriage Reception","from_date":"2019-02-06 10:00","to_date":"2019-02-08 05:00","family_types":["1","2"],"streets":["1","2"],"sent_event_to_all_members":"1","latitude":"8.565985","longitude":"78.12379299999998","user_id":"1","status":"1"},"event_invitation_info":{"file_name":"invitation.png","image_data":"data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAASwAAADggg=="},"event_dynamic_info":{"1":"Birthday Party","2":"3","4":"07/02/2019","5":"Sample Description"}}';

        $input_data = json_decode($json_input, TRUE);
        if (!empty($input_data)) {
            $event_info = $input_data['event_info'];
            $event_invitation_info = $input_data['event_invitation_info'];
            $event_dynamic_info = $input_data['event_dynamic_info'];

            $is_event_name_available = $this->event_api_model->is_event_name_available($event_info['event_name']);
            if ($is_event_name_available) {
                $time = date('Y-m-d H:i:s');
                $family_types = $event_info['family_types'];
                $streets = $event_info['streets'];
                $event_info['family_types'] = implode(',', $family_types);
                $event_info['streets'] = implode(',', $streets);
    ...
    function api_edit_event() {
        $json_input = file_get_contents('php://input'); // JSON Input
        //$json_input = '{"event_id":"5","event_info":{"event_type_id":"1","event_name":"Vijay Marriage Reception","from_date":"2019-02-06 10:00","to_date":"2019-02-08 05:00","family_types":["1","3"],"streets":["1","2"],"sent_event_to_all_members":"1","latitude":"8.565985","longitude":"78.12379299999998","user_id":"1","status":"1"},"event_invitation_info":{"file_name":"sample.png","image_data":"data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAOEAAADgg=="},"event_dynamic_info":{"1":"DJ Birthday Party","2":"3","4":"05/02/2019","5":"Test Description"}}';
